...
现有独立 `PowerControl()` 把 `MecanumCalculate()` 的四轮结果当作 `rpm`，而统一后的 `DJIMotorControl()` 以度每秒计算速度环。迁移时在 `chassis.c` 中把四轮目标乘以 `6.0f` 后再调用 `DJIMotorSetRef()`，并把当前纯 P 速度环的 `Kp` 从 `4.5` 调整为 `0.75`。这样目标与反馈同时放大六倍后，速度环输出与迁移前保持等价。功率算法仍把目标和反馈除以 `6.0f`，统一使用 `rpm`。
...
功率算法中的速度统一使用 `rpm`，DJI 通用驱动内部的速度闭环继续使用度每秒；两者只在挂接边界做固定的 `6.0f` 换算。电流输入和输出统一使用 DJI 电调原始命令值。首版不将制动回馈功率计为负功率，避免高估可用功率。
...
1. 读取所有电机当前参考值和反馈；
2. 计算角度环、速度环、电流环和前馈；
3. 在速度环入口保存功控组电机的目标速度和反馈速度，并换算为 `rpm`；
4. 保存所有电机的原始输出，不发送；
5. 计算四轮功率额度；
6. 修改功控组四个原始输出；
7. 应用反馈反向设置；
